...
switch($opcion){
    case 1: //alta
        ...
        $consulta = "INSERT INTO reservas (fechaentrada, fechasalida, idcliente, idhabitacion, cantidad, costo) VALUES('$fechaentrada', '$fechasalida', '$idcliente', '$tipo', '$cantidad', '$costo' ) ";			 
        $resultado = $conexion->prepare($consulta);
        $resultado->execute(); 

        $consulta = "SELECT idreserva, nombre, fechaentrada, fechasalida, cantidad, costo FROM clientes INNER JOIN reservas ON clientes.idcliente = reservas.idcliente ORDER BY idreserva DESC LIMIT 1";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
    ...
    case 2: //modificación
        ...
        $consulta = "UPDATE reservas SET fechaentrada='$fechaentrada', fechasalida='$fechasalida' , idcliente='$id', idhabitacion='$tipo', cantidad='$cantidad', costo='$costo' WHERE idreserva='$id' ";		
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();     
        
        
        $consulta = "SELECT idreserva, nombre, fechaentrada, fechasalida,telefono,cantidad, costo FROM clientes INNER JOIN reservas ON clientes.idcliente = reservas.idcliente WHERE clientes.idcliente='$id' ";       
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;        
